...
export default function ListCenter(props) {
  ...
  return (
    <div className="div_list_cty">
      {searchBusinessName?.filter((item) => item.status === "active").length !==0  
      || searchBusinessCareer?.filter((item) => item.status === "active").length !==0
      || searchBusinessProduct?.filter((item) => item.status === "active").length !==0 ? (
        <Fragment>
          <Fragment>
            {searchBusinessName
